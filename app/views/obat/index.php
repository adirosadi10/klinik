  ...
  <tbody>
    ...
    foreach ($data['obat'] as $obat) { ?>
      <tr>
        ...
        <td><?= $obat['harga']; ?></td>
        <td>
          <span class="badge bg-success"><a style="text-decoration: none;color:white" class="tampilModalUbah" data-id="<?= $obat['id']; ?>" data-bs-toggle="modal" data-bs-target="#tambah">Edit</a></span> |
          <span class="badge bg-danger"><a style="text-decoration: none;color:white" href="<?= BASE_URL; ?>/obat/delete/<?= $obat['id']; ?>" onclick="return confirm('Apakah anda yakin akan menghapus data?');">Hapus</a></span>
        </td>
      </tr>
    <?php } ?>
  </tbody>
